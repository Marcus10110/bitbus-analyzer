...
> "libBitbusAnalyzer.so" cannot be opened because th developer cannot be verified.
...
```sh
xattr -r -d com.apple.quarantine libBitbusAnalyzer.so
```
...
cmake .. -A x64
cmake --build .
:: built analyzer will be located at SampleAnalyzer\build\Analyzers\Debug\BitbusAnalyzer.dll
```
...
cmake ..
cmake --build .
# built analyzer will be located at SampleAnalyzer/build/Analyzers/BitbusAnalyzer.so
```
